...
/*
    Evaluate Set Position with right Turn
...
SetPositionNode::SetPositionNode(): DefaultRobotNode("set_position_turn_right_node", ROBOT_NAME) {
...
void SetPositionNode::step_callback(const std_msgs::msg::Empty::SharedPtr) {
    ...
    if ( 
        ...
        has_finished = true;
    } else if ( this->direction == RIGHT && this->current_position_left >= 4 * M_PI) {
        this->direction = FORWARD;
        has_finished = true;
    }
